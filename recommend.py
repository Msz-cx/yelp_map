...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        loca = restaurant_location(restaurant)
        lst.append([find_closest(loca, centroids), restaurant])
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        lati.append(restaurant_location(restaurant)[0])
        longi.append(restaurant_location(restaurant)[1])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum((i - mean(xs)) * (j - mean(ys)) for i,j in xy_s)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        lst.append(find_predictor(user, reviewed, fn))
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            dic[name] = user_rating(user, name)
        else:
            dic[name] = predictor(restaurant)
    return dic
    # END Question 9
